...
import type { GetUserDetailsWeb } from "@/services/api/fetch_user_data/fetchUserData.types";

export type { GetUserDetailsWeb, UserModel } from "@/services/api/fetch_user_data/fetchUserData.types";
...
const fetchUserData = async () =>
  await fetchWithOfflineSnapshot(USER_SNAPSHOT_ID, async () => {
    const rawUser = await rawRequest<GetUserDetailsWeb>({
      auth: true,
      path: "/api/user",
    });
